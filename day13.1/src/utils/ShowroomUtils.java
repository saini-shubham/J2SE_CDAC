...
public class ShowroomUtils {
	...
	//add a static method to apply discount to the vehicles by date and color
	public static void applyDiscount(String date,String clr,Double discount,ArrayList<Vehicle> store) throws ParseException, VehicleValidationException {
		Date d1=validateDate(date);
		Color color=validateColor(clr);
		for(Vehicle v:store) {
			if(v.getManufactureDate().before(d1)&&v.getColor()==color)   //== works in enum case as there is exactly one copy of each colour
